...
export class RefusComponent implements OnInit {
  ...
  columnDefs: ColDef[] = [
    ...
    // REVISED "Avis des membres" COLUMN
    // REVISED "Avis des membres" COLUMN (compact result and name)
    {
      ...
      cellRenderer: (params: ICellRendererParams) => {
        ...
        return params.value
          .map((dec: any) => {
            ...
            return `<span style="color: red ; font-weight: bold;">${decisionText}</span>`;
          })
          .join("<br>");
      },
      minWidth: 80,
  ...
  getDossiersWithVisaRefus(): void {
    this.isLoading = true;
    this.errorMessage = null;

    this.dossierService.getVisaRefusDecisions().pipe(
